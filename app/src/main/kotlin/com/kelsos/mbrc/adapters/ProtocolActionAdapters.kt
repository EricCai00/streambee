...
class TrackChangeNotifierImpl(
  ...
  override suspend fun requestTrackDetails(path: String?) {
    val requestedPath = path?.takeIf { it.isNotBlank() }
    // Unqualified requests are generated by remote track-change events. They
    // must not overwrite a local queue item. An explicit path request is the
    // detail-sheet refresh for that local item and is allowed through.
    if (requestedPath == null && localPlaybackController.hasLocalPlayback) return
    withContext(dispatchers.network) {
      runCatching {
        val details = playbackApi.getTrackDetails(requestedPath)
        val stillShowingRequestedTrack = requestedPath == null ||
          appState.playingTrack.value.path == requestedPath
        val canPublish = if (requestedPath == null) {
          !localPlaybackController.hasLocalPlayback
        } else {
          stillShowingRequestedTrack
        }
        if (canPublish) {
